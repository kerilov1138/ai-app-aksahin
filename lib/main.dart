    ...
    try {
      ...
      _failsafeTimer = Timer(const Duration(seconds: 15), () {
        if (_isLoading && mounted && _errorMessage == null) {
          _showError("Initialization Timeout: The app is taking too long to load or the server failed to bind.");
        }
      });

      // 5. Load from Hardened Localhost
  ...
  Future<void> _startHardenedServer() async {
    ...
    try {
      ...
      _server!.listen((HttpRequest request) async {
        try {
          // Decode URL to handle spaces/Turkish characters
          ...
          // Force Offline Security & Stability Headers
          request.response.headers.add('Access-Control-Allow-Origin', '*');
          request.response.headers.add('Access-Control-Allow-Methods', 'GET, OPTIONS');
          request.response.headers.contentLength = bytes.length;
          
          // Enhanced MIME Mapping
          String ext = path.split('.').last.toLowerCase();
          switch (ext) {
            case 'html': request.response.headers.contentType = ContentType.html; break;
            case 'js':
            case 'tsx':
            case 'jsx': request.response.headers.contentType = ContentType.parse('application/javascript; charset=utf-8'); break;
